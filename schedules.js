...
function leaveForDay(employeeId,key=activeDay?.key){if(!key)return null;const date=new Date(`${key}T12:00:00`),employeeLeaves=(ctx?.state?.leaves||[]).filter(leave=>leave.employeeId===employeeId),latestWeekly=employeeLeaves.filter(leave=>leave.type==="weekly").sort((a,b)=>Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0))[0],matching=employeeLeaves.filter(leave=>leave.type!=="weekly"&&leave.startDate<=key&&leave.endDate>=key&&!(leave.skipEnabled&&Number(leave.skipWeekday)===date.getDay()));if(latestWeekly&&weeklyDaysFor(latestWeekly).includes(date.getDay()))matching.push(latestWeekly);return matching.sort((a,b)=>(b.duration==="full")-(a.duration==="full")||Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0))[0]||null;}
