...
const fallbackTaskNames={"عامل":"Worker","استقبال":"Reception","فران":"Baker","مخازن":"Warehouse","سكرتاريا":"Secretary","عامل مطبخ":"Kitchen Staff","فراش":"Office Attendant","أمين صندوق":"Cashier","تنظيف":"Cleaning","مبيعات":"Sales","حلويات":"Sweets","إعداد خلطات":"Mix Preparation","اعداد خلطات":"Mix Preparation","محاسب":"Accountant","سائق":"Driver","خلاط":"Mixer","مخبز":"Bakery"};
...
function translateCompoundText(clean){return clean.split(/([+/،])/).map(part=>{const token=part.trim();if(!token||token==="+"||token==="/"||token==="،")return part;return fallbackTaskNames[token]||latinizeArabic(token)||token;}).join("").replace(/\s+/g," ").trim();}
function fallbackText(value=""){const clean=String(value).trim();if(!clean)return"Work task";const directMap={...fallbackTaskNames,"إجازة أسبوعية":"Weekly leave","إجازة سنوية":"Annual leave","إجازة مرضية":"Sick leave","نصف يوم":"Half day","عام":"General","دوام":"Shift"};return directMap[clean]||(/[\+/،]/.test(clean)?translateCompoundText(clean):latinizeArabic(clean))||"Work task";}
function fallbackTranslation(){const employeeNames=Object.fromEntries(ctx.state.employees.map(employee=>[employee.id,latinizeArabic(firstName(employee.fullName))||"Employee"]));return{title:`Branch Schedule — ${latinizeArabic(schedule.dayName)} ${displayDate(activeDay.date)}`,branchNames:fallbackBranchNames,employeeNames,tasks:Object.fromEntries(assignments().map(item=>[item.id,item.tasks.map(fallbackText)])),notes:Object.fromEntries(notes().map(note=>[note.id,fallbackText(note.text)]))};}
