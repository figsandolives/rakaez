...
const fallbackTaskNames={"عامل":"Worker","استقبال":"Reception","فران":"Baker","مخازن":"Warehouse","سكرتاريا":"Secretary","عامل مطبخ":"Kitchen Staff","فراش":"Office Attendant","أمين صندوق":"Cashier","تنظيف":"Cleaning","مبيعات":"Sales"};
...
function fallbackText(value=""){const clean=String(value).trim();return fallbackTaskNames[clean]||latinizeArabic(clean)||"Work task";}
...
async function downloadPdf(){if(!schedule.translation||schedule.translationError){ctx.showToast("لا يمكن تحميل PDF حتى تكتمل الترجمة الصحيحة.");return;}const root=$("#pdf-render-root");root.innerHTML=pdfPage("ar")+pdfPage("en");const pages=[...root.querySelectorAll(".pdf-page")],{jsPDF}=window.jspdf,pdf=new jsPDF({orientation:"landscape",unit:"mm",format:"a4"});for(let index=0;index<pages.length;index++){const canvas=await window.html2canvas(pages[index],{scale:2,backgroundColor:"#ffffff",useCORS:true});if(index)pdf.addPage("a4","landscape");pdf.addImage(canvas.toDataURL("image/png"),"PNG",0,0,297,210);}pdf.save(`جدول دوام الأفرع يوم ${schedule.dayName} تاريخ ${displayDate(activeDay.date)}.pdf`);root.innerHTML="";}
