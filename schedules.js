...
function emptySchedule(day){return{dateKey:day.key,dayName:day.name,date:day.date.toISOString(),assignments:{},notes:{},overtime:{},published:false,translation:null,updatedAt:Date.now()};}
...
function overtimeHours(employeeId){return Math.max(0,Number(schedule?.overtime?.[employeeId]?.hours||0));}
function leaveTypeLabel(leave){return({weekly:"إجازة أسبوعية",sick:"إجازة مرضية",annual:"إجازة سنوية",gregorian_new_year:"رأس السنة الميلادية",hijri_new_year:"رأس السنة الهجرية",national_day:"العيد الوطني",eid_al_fitr:"عيد الفطر",eid_al_adha:"عيد الأضحى"})[leave?.type]||"إجازة";}
function englishLeaveTypeLabel(leave){return({weekly:"Weekly day off",sick:"Sick leave",annual:"Annual leave",gregorian_new_year:"Gregorian New Year",hijri_new_year:"Hijri New Year",national_day:"National Day",eid_al_fitr:"Eid al-Fitr",eid_al_adha:"Eid al-Adha"})[leave?.type]||"Leave";}
...
function leaveForDay(employeeId,key=activeDay?.key){if(!key)return null;const date=new Date(`${key}T12:00:00`),employeeLeaves=(ctx?.state?.leaves||[]).filter(leave=>leave.employeeId===employeeId),latestWeekly=employeeLeaves.filter(leave=>leave.type==="weekly").sort((a,b)=>Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0))[0],matching=employeeLeaves.filter(leave=>!leave.pending&&leave.type!=="weekly"&&(Array.isArray(leave.selectedDates)&&leave.selectedDates.length?leave.selectedDates.includes(key):leave.startDate<=key&&leave.endDate>=key)&&!(leave.skipEnabled&&Number(leave.skipWeekday)===date.getDay()));if(latestWeekly&&weeklyDaysFor(latestWeekly).includes(date.getDay()))matching.push(latestWeekly);return matching.sort((a,b)=>(b.duration==="full")-(a.duration==="full")||Number(b.updatedAt||b.createdAt||0)-Number(a.updatedAt||a.createdAt||0))[0]||null;}
function availableWorkHours(employee){const leave=leaveForDay(employee.id),base=Math.max(0,Number(employee.dailyHours||0)),overtime=overtimeHours(employee.id);if(leave?.duration==="full")return overtime;if(leave?.duration==="half")return(base/2)+overtime;return base+overtime;}
...
function renderDayWorkspace(){
  ...
  poolHead.insertAdjacentHTML("afterend",`<label class="hide-finished-toggle"><input id="hide-finished-employees" type="checkbox" ${hideFinishedEmployees?"checked":""}><span>إخفاء المنتهين</span></label><button type="button" id="add-overtime" class="add-overtime-button"><i aria-hidden="true">♙</i><span>دوام إضافي</span></button>`);
...
function employeePoolCard(employee){const leave=leaveForDay(employee.id),remaining=remainingHours(employee),overtime=overtimeHours(employee.id),fullLeave=leave?.duration==="full"&&remaining<=0,done=remaining<=0&&!fullLeave,photo=employee.photoDataUrl||employee.photoUrl;const hours=leave&&!overtime?`<strong class="pool-leave-label">${esc(leaveTypeLabel(leave))}${leave.duration==="half"?" · نصف يوم":""}</strong>`:`<strong class="pool-hours ${overtime?"overtime-hours":""}"><b>${formatHours(remaining)}</b><small>${overtime?`متضمن ${formatHours(overtime)} ساعات إضافية`:"ساعة متبقية"}</small></strong>`;return`<article class="pool-card ${done?"complete":""} ${fullLeave?"leave-full":""} ${leave?.duration==="half"?"leave-half":""} ${overtime?"has-overtime":""}" draggable="${done||fullLeave?"false":"true"}" data-employee="${employee.id}"><div class="pool-avatar">${photo?`<img src="${esc(photo)}" alt="">`:initials(employee.fullName)}</div><div><b>${esc(employee.fullName)}</b><small>${esc(employee.jobTitle||"")}</small></div>${hours}</article>`;}
...
function bindWorkspaceEvents(){
  ...
  $("#add-overtime").onclick=openOvertimeModal;
...
function openOvertimeModal(){
  ...
  root.innerHTML=`<div class="modal-backdrop overtime-backdrop"><section class="modal overtime-modal" role="dialog" aria-modal="true" aria-labelledby="overtime-title"><header><div><span>جدول الدوامات</span><h3 id="overtime-title">إضافة دوام إضافي</h3></div><button type="button" class="modal-close" aria-label="إغلاق">×</button></header><form id="overtime-form" class="overtime-form"><label>اسم الموظف<div class="overtime-search"><input id="overtime-employee-search" autocomplete="off" placeholder="ابحث عن الموظف ثم اختره"><div id="overtime-suggestions"></div></div></label><div id="overtime-selected" class="overtime-selected hidden"></div><label>عدد الساعات<input id="overtime-hours" class="english-number" inputmode="decimal" placeholder="مثال: 2.5" disabled required></label><p id="overtime-error" class="form-message error hidden"></p><footer><span></span><div><button type="button" class="secondary modal-cancel">إلغاء</button><button class="primary" disabled>اعتماد الإضافة</button></div></footer></form></section></div>`;
  const close=()=>root.innerHTML="";
  root.querySelector(".modal-close").onclick=root.querySelector(".modal-cancel").onclick=close;
  root.querySelector(".overtime-backdrop").onclick=event=>{if(event.target.classList.contains("overtime-backdrop"))close();};
  let selectedEmployeeId="";
  const search=$("#overtime-employee-search"),suggestions=$("#overtime-suggestions"),selected=$("#overtime-selected"),hours=$("#overtime-hours"),submit=root.querySelector("button.primary"),error=$("#overtime-error");
  const renderSuggestions=query=>{const normalized=query.trim();const matches=ctx.state.employees.filter(employee=>!normalized||employee.fullName.includes(normalized)).slice(0,8);suggestions.innerHTML=matches.map(employee=>{const photo=employee.photoDataUrl||employee.photoUrl;return`<button type="button" data-overtime-employee="${esc(employee.id)}"><span>${photo?`<img src="${esc(photo)}" alt="">`:initials(employee.fullName)}</span><div><b>${esc(employee.fullName)}</b><small>${esc(employee.jobTitle||"")}</small></div></button>`;}).join("")||'<p>لا توجد نتائج</p>';document.querySelectorAll("[data-overtime-employee]").forEach(button=>button.onclick=()=>{selectedEmployeeId=button.dataset.overtimeEmployee;const employee=employeeById(selectedEmployeeId),photo=employee.photoDataUrl||employee.photoUrl;search.value=employee.fullName;suggestions.innerHTML="";selected.innerHTML=`<span>${photo?`<img src="${esc(photo)}" alt="">`:initials(employee.fullName)}</span><b>${esc(employee.fullName)}</b><small>سيُتاح له دوام إضافي في هذا اليوم</small>`;selected.classList.remove("hidden");hours.disabled=false;hours.focus();submit.disabled=false;});};
  search.onfocus=()=>renderSuggestions(search.value);
  search.oninput=()=>{selectedEmployeeId="";selected.classList.add("hidden");hours.disabled=true;submit.disabled=true;renderSuggestions(search.value);};
  hours.oninput=()=>{hours.value=latinDigits(hours.value).replace(/[^0-9.]/g,"").replace(/(\..*)\./g,"$1");};
  $("#overtime-form").onsubmit=async event=>{event.preventDefault();const value=Number(hours.value),employee=employeeById(selectedEmployeeId);if(!employee){error.textContent="اختر الموظف أولاً.";error.classList.remove("hidden");return;}if(!Number.isFinite(value)||value<=0){error.textContent="اكتب عدد الساعات بصورة صحيحة، مثل 2.5.";error.classList.remove("hidden");return;}schedule.overtime=schedule.overtime||{};const previous=overtimeHours(employee.id);schedule.overtime[employee.id]={employeeId:employee.id,hours:previous+value,updatedAt:Date.now()};await persistSchedule({invalidate:true});close();renderDayWorkspace();ctx.showToast(`تمت إضافة ${formatHours(value)} ساعات دوام إضافي لـ ${firstName(employee.fullName)}.`);};
}

function openAssignmentModal(employeeId,branchId,assignmentId=""){
  const existing=assignmentId?schedule.assignments?.[assignmentId]:null,employee=employeeById(employeeId),branch=BRANCHES.find(item=>item.id===branchId),leave=employee&&leaveForDay(employeeId),remaining=employee&&remainingHours(employee),available=Math.max(0,Number(remaining||0)+assignmentWorkHours(existing));if(!employee||!branch||(!existing&&remaining<=0))return;
  const leaveNotice=leave?`<div class="assignment-half-notice">${esc(leaveTypeLabel(leave))}${leave.duration==="half"?" · نصف يوم":" · تم تفعيل الدوام الإضافي"}</div>`:"",editing=Boolean(existing),initialBreak=Math.max(0,Number(existing?.breakMinutes||0));
  const existingTaskTranslations=Array.isArray(existing?.taskTranslations)?existing.taskTranslations:[];
  document.querySelector("#modal-root").innerHTML=`<div class="modal-backdrop"><section class="modal work-segment-modal" role="dialog" aria-modal="true"><header><div><span>${esc(branch.name)}</span><h3>${editing?"تعديل دوام":"إضافة دوام"} ${esc(firstName(employee.fullName))}</h3></div><button type="button" class="modal-close">×</button></header><form id="schedule-assignment-form">${leaveNotice}<div class="assignment-remaining">متبقي للموظف <b>${formatHours(available)} ساعات عمل</b></div><div class="time-grid"><label>من الساعة${modalTimePicker("schedule-from",existing?.from||"08:00")}</label><span>←</span><label>إلى الساعة${modalTimePicker("schedule-to",existing?.to||"10:00")}</label></div><div class="modal-duration">مدة الفترة: <b id="schedule-duration">2 ساعات</b></div><section class="assignment-break-config"><label class="assignment-break-toggle"><input id="schedule-break-enabled" type="checkbox" ${initialBreak?"checked":""}><span>يوجد وقت بريك</span></label><label id="schedule-break-minutes-wrap" class="assignment-break-minutes ${initialBreak?"":"hidden"}">مدة البريك بالدقائق<input id="schedule-break-minutes" inputmode="numeric" maxlength="3" value="${initialBreak||""}" placeholder="مثال: 30"></label></section><div class="tasks-head"><label>مهام العمل</label><button type="button" id="schedule-add-task" class="mini-add">＋ مهمة إضافية</button></div><div id="schedule-tasks">${(existing?.tasks?.length?existing.tasks:[""]).map(taskRow).join("")}</div><section class="item-translation-panel"><div class="item-translation-head"><div><b>الترجمة الإنجليزية للمهام</b><span>اختر الترجمة بالذكاء الاصطناعي أو اكتبها يدوياً لكل مهمة</span></div><div class="item-translation-actions"><button type="button" id="translate-assignment" class="secondary">ترجمة</button><button type="button" id="manual-translate-assignment" class="secondary">ترجمة يدوية</button></div></div>${translationStatus()}<div id="assignment-translations">${existingTaskTranslations.length?existingTaskTranslations.map((item,index)=>`<label class="task-translation-field">ترجمة المهمة ${index+1}<input dir="ltr" value="${esc(item.text||"")}" data-task-translation-source="${esc(item.source||"")}"></label>`).join(""):""}</div></section><p id="schedule-modal-error" class="form-message error hidden"></p><footer><span></span><div><button type="button" class="secondary modal-cancel">إلغاء</button><button class="primary" id="save-assignment">${editing?"حفظ التعديلات":"إضافة إلى الجدول"}</button></div></footer></form></section></div>`;
  ...
  $("#manual-translate-assignment").onclick=()=>{const tasks=taskValues(),error=$("#schedule-modal-error");if(!tasks.length){error.textContent="اكتب مهمة واحدة على الأقل قبل إضافة الترجمة اليدوية.";error.classList.remove("hidden");return;}error.classList.add("hidden");$("#assignment-translations").innerHTML=tasks.map((task,index)=>`<label class="task-translation-field">ترجمة المهمة ${index+1}<input dir="ltr" placeholder="اكتب الترجمة الإنجليزية" data-task-translation-source="${esc(task)}"></label>`).join("");$("#assignment-translations input")?.focus();};
...
async function translateItems(items,label){
  ...
  try{
    if(!localAI?.url||!localAI?.model)throw new Error("جارٍ الاتصال بخادم الترجمة. أعد المحاولة بعد لحظات.");
    ...
    let payload;try{payload=await response.json();}catch{throw new Error("خادم الترجمة أعاد استجابة غير صالحة.");}
    if(!response.ok||payload?.ok===false)throw new Error(payload?.message||payload?.error||"تعذر اتصال Ollama المحلي بخدمة الترجمة.");
    ...
    throw new Error(error?.message||"تعذر الاتصال بنموذج الذكاء الاصطناعي على خادم الترجمة.");
